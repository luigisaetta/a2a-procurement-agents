...
Docker Compose persistence note:

- the Purchase Order Agent stores the A2A registration payload in `purchase_orders`
- `purchase_order_id` may be generated by the agent using a database-backed sequence table
- `(request_id, offer_id)` is the idempotency key for demo persistence
- the Docker Compose table may store denormalized business codes such as `plant_code` and `material_code` so purchase order registration does not depend on supplier offer rows being preloaded into the demo database
